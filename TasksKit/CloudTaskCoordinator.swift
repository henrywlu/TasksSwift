...
//  Created by Henry W. Lu on 5/17/15.
    ...
    Abstract:
    The `CloudTaskCoordinator` class handles querying for and interacting with tasks stored as files in iCloud Drive.
*/

import Foundation

@objc public class CloudTaskCoordinator: TaskCoordinator {
    // MARK: Properties
    ...
    /// A private, local queue to `CloudTaskCoordinator` that is used to ensure serial accesss to `documentsDirectory`.
    private let documentsDirectoryQueue = dispatch_queue_create("com.locust123.tasker.cloudtaskcoordinator", DISPATCH_QUEUE_CONCURRENT)
    ...
    @objc private func metadataQueryDidFinishGathering(notifcation: NSNotification) {
        ...
        delegate?.taskCoordinatorDidUpdateContents(insertedURLs: insertedURLs, removedURLs: [], updatedURLs: [])
    ...
    */
    @objc private func metadataQueryDidUpdate(notification: NSNotification) {
        ...
        delegate?.taskCoordinatorDidUpdateContents(insertedURLs: insertedURLs, removedURLs: removedURLs, updatedURLs: updatedURLs)
